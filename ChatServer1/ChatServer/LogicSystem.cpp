...
void LogicSystem::DealMsg()
{
	while (1) {
		...
		{
			//即使是有效的唤醒（即确实有线程调用了 notify_one 或 notify_all），也不能保证条件在唤醒时仍然成立。可能有多个线程在等待同一个条件变量，当其中一个线程被唤醒并占用了锁后，其他线程也会被唤醒，但条件可能已经被第一个线程改变了。因此，每个被唤醒的线程都需要重新检查条件。
...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
	...
	//从数据库获取申请列表 先从内存层面 缓存加载层面 数据库加载层面
