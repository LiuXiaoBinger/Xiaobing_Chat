  ...
  ,_load_pending(false)
{
    ...
    //链接对端同意认证后通知的信号
    connect(TcpMgr::Get_M_ptr().get(), &TcpMgr::sig_add_auth_friend,this,
...
void ContactUserList::addContactUserList()
{
    ...
    item->setSizeHint(groupTip->sizeHint());//这行代码将 QListWidgetItem 的大小提示设置为 groupTip 小部件的大小提示。这确保了列表项的大小与其包含的小部件的大小相匹配。
    this->addItem(item);
    ...
    //加载后端发送过来的好友列表
    auto con_list = UserMgr::Get_M_ptr()->GetConListPerPage();
...
bool ContactUserList::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (watched == this->viewport() && event->type() == QEvent::Wheel) {
        ...
        if (maxScrollValue - currentValue <= 0) {
            ...
            auto b_loaded = UserMgr::Get_M_ptr()->IsLoadChatFin();
            if(b_loaded){
                return true;
            }
...
void ContactUserList::slot_add_auth_firend(std::shared_ptr<AuthInfo> auth_info)
{
    ...
    bool isFriend = UserMgr::Get_M_ptr()->CheckFriendById(auth_info->_uid);
    if(isFriend){
        return;
    }
    // 在 groupitem 之后插入新项
...
void ContactUserList::slot_auth_rsp(std::shared_ptr<AuthRsp> auth_rsp)
{
    ...
    bool isFriend = UserMgr::Get_M_ptr()->CheckFriendById(auth_rsp->_uid);
    if(isFriend){
        return;
    }
    // 在 groupitem 之后插入新项
