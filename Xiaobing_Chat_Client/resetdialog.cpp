    ...
    ui(new Ui::ResetDialog)
{
    ...
    connect(ui->m_pwd_edit, &QLineEdit::editingFinished, this, [this](){
        checkPassValid();
    });
    ...
    //连接reset相关信号和注册处理回调
    initHandlers();
    connect(httpController::Get_M_ptr().get(), &httpController::sig_reset_mod_finish, this,
...
void ResetDialog::on_m_return_btn_clicked()
{
    qDebug() << "sure btn clicked ";
    emit switchLogin();
}

void ResetDialog::on_m_varify_btn_clicked()
{
    ...
    httpController::Get_M_ptr()->PostHttpReq(QUrl(gate_url_prefix+"/get_varifycode"),
...
void ResetDialog::on_m_sure_btn_clicked()
{
    ...
    json_obj["passwd"] = xorString(ui->m_pwd_edit->text());
    json_obj["varifycode"] = ui->m_varify_edit->text();
    httpController::Get_M_ptr()->PostHttpReq(QUrl(gate_url_prefix+"/reset_pwd"),
