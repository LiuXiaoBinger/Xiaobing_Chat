...
void SearchList::slot_item_clicked(QListWidgetItem *item)
{
    ...
    if(itemType == ListItemType::ADD_USER_TIP_ITEM){
        ...
        //发送tcp请求给chat server
        emit TcpMgr::Get_M_ptr()->sig_send_data(ReqId::ID_SEARCH_USER_REQ, jsonData);
        return;
    }
    CloseFindDlg();
}
...
void SearchList::slot_user_search(std::shared_ptr<SearchInfo> si)
{
    ...
    if (si == nullptr) {
        ...
        auto self_uid = UserMgr::Get_M_ptr()->GetUid();
        if (si->_uid == self_uid) {
                 return;
        }
        //此处分两种情况，一种是搜多到已经是自己的朋友了，一种是未添加好友
        //查找是否已经是好友
        bool bExist = UserMgr::Get_M_ptr()->CheckFriendById(si->_uid);
        if(bExist){
                //此处处理已经添加的好友，实现页面跳转
            //跳转到聊天界面指定的item中
