...
void TcpMgr::initHandlers()
{
    ...
    _handlers.insert(ID_CHAT_LOGIN_RSP, [this](ReqId id, int len, QByteArray data){
        ...
        UserMgr::Get_M_ptr()->SetUserInfo(user_info);
        UserMgr::Get_M_ptr()->SetToken(jsonObj["token"].toString());
        //添加好友申请表
        if(jsonObj.contains("apply_list")){
            UserMgr::Get_M_ptr()->AppendApplyList(jsonObj["apply_list"].toArray());
        }

        //添加好友列表
        if (jsonObj.contains("friend_list")) {
            UserMgr::Get_M_ptr()->AppendFriendList(jsonObj["friend_list"].toArray());
        }
        //加载头像
    ...
    _handlers.insert(ID_SEARCH_USER_RSP, [this](ReqId id, int len, QByteArray data) {
        ...
        // 检查转换是否成功
        if (jsonDoc.isNull()) {
            qDebug() << "SEARCH_USER_RSP Failed to create QJsonDocument.";
            return;
        }
    ...
    _handlers.insert(ID_NOTIFY_ADD_FRIEND_REQ, [this](ReqId id, int len, QByteArray data) {
        ...
        // 检查转换是否成功
        if (jsonDoc.isNull()) {
            qDebug() << "ADD_FRIEND to create QJsonDocument.";
            return;
        }
        ...
        if (err != ErrorCodes::SUCCESS) {
         ...
         QString nick = jsonObj["nick"].toString();//昵称
