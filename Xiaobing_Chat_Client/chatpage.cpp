...
{
    auto self_info = UserMgr::Get_M_ptr()->GetUserInfo();
    ChatRole role;
    //todo... 添加聊天显示
    if (msg->_from_uid == self_info->_uid) {
        ...
        role = ChatRole::Other;
        ChatItemBase* pChatItem = new ChatItemBase(role);
        auto friend_info = UserMgr::Get_M_ptr()->GetFriendById(msg->_from_uid);
...
void ChatPage::on_send_btn_clicked()
{
    ...
    auto user_info = UserMgr::Get_M_ptr()->GetUserInfo();
    ...
    for(int i=0; i<msgList.size(); ++i)
    {
        ...
        if(type == "text")
        {
            ...
            if(txt_size + msgList[i].content.length()> 1024){
                ...
                textObj = QJsonObject();
                //发送tcp请求给chat server
                emit TcpMgr::Get_M_ptr()->sig_send_data(ReqId::ID_TEXT_CHAT_MSG_REQ, jsonData);
            }
