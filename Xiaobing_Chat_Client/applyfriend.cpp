    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    this->setModal(true);
    ui->name_ed->setPlaceholderText(tr("lyf"));
...
{
    _si = si;
    auto applyname = UserMgr:: Get_M_ptr()->GetName();
...
void ApplyFriend::SlotApplySure()
{
    ...
    //发送请求逻辑
    QJsonObject jsonObj;
    auto uid = UserMgr::Get_M_ptr()->GetUid();
    jsonObj["uid"] = uid;
    ...
    //发送tcp请求给chat server
    emit TcpMgr::Get_M_ptr()->sig_send_data(ReqId::ID_ADD_FRIEND_REQ, jsonData);
    this->hide();
    deleteLater();
}
