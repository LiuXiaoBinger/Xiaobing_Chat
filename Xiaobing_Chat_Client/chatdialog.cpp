    ...
    _state(ChatUIMode::ChatMode),_last_widget(nullptr),_cur_chat_uid(0)
{
    ...
    //模拟加载自己头像
    //QString head_icon = UserMgr::Get_M_ptr()->GetIcon();
    ...
    //为searchlist 设置search edit，将编辑框指针给到search_list对象类中方便透传信息
    ui->search_list->SetSearchEdit(ui->search_edit);

    //连接申请添加好友信号
    connect(TcpMgr::Get_M_ptr().get(), &TcpMgr::sig_friend_apply, this, &ChatDialog::slot_apply_friend);
    //连接认证添加好友信号
    connect(TcpMgr::Get_M_ptr().get(), &TcpMgr::sig_add_auth_friend, this, &ChatDialog::slot_add_auth_friend);
    //链接自己认证回复信号
    connect(TcpMgr::Get_M_ptr().get(), &TcpMgr::sig_auth_rsp, this,
    ...
    //连接对端消息通知
    connect(TcpMgr::Get_M_ptr().get(), &TcpMgr::sig_text_chat_msg,
...
void ChatDialog::slot_append_send_chat_msg(std::shared_ptr<TextChatData> msgdata) {
    ...
    if (itemType == CHAT_USER_ITEM) {
        ...
        UserMgr::Get_M_ptr()->AppendFriendChatMsg(_cur_chat_uid,msg_vec);
        return;
    }
}
...
void ChatDialog::slot_text_chat_msg(std::shared_ptr<TextChatMsg> msg)
{
    ...
    if(find_iter != _chat_items_added.end()){
        ...
        UserMgr::Get_M_ptr()->AppendFriendChatMsg(msg->_from_uid,msg->_chat_msgs);
        return;
    }
    ...
    //查询好友信息
    auto fi_ptr = UserMgr::Get_M_ptr()->GetFriendById(msg->_from_uid);
...
void ChatDialog::slot_auth_rsp(std::shared_ptr<AuthRsp> auth_rsp)
{
    ...
    //判断如果已经是好友则跳过
    auto bfriend = UserMgr::Get_M_ptr()->CheckFriendById(auth_rsp->_uid);
    if(bfriend){
        return;
    }

    UserMgr::Get_M_ptr()->AddFriend(auth_rsp);
...
void ChatDialog::slot_apply_friend(std::shared_ptr<AddFriendApply> apply)
{
   ...
   bool b_already = UserMgr::Get_M_ptr()->AlreadyApply(apply->_from_uid);
   if(b_already){
        return;
   }

   UserMgr::Get_M_ptr()->AddApplyList(std::make_shared<ApplyInfo>(apply));
...
void ChatDialog::slot_add_auth_friend(std::shared_ptr<AuthInfo> auth_info) {
    ...
    //判断如果已经是好友则跳过
    auto bfriend = UserMgr::Get_M_ptr()->CheckFriendById(auth_info->_uid);
    if(bfriend){
        return;
    }

    UserMgr::Get_M_ptr()->AddFriend(auth_info);
...
void ChatDialog::setHeadlb()
{
    QString headfile=UserMgr::Get_M_ptr()->GetUserInfo()->_icon;
    if(headfile=="")
    {
        headfile=":/res/head_1.jpg"; // 加载图片
    }

    QPixmap pixmap(headfile); // 加载图片
...
void ChatDialog::addChatUserList()
{
    ...
    auto friend_list = UserMgr::Get_M_ptr()->GetChatListPerPage();
    if (friend_list.empty() == false) {
        ...
        //更新已加载条目
        UserMgr::Get_M_ptr()->UpdateChatLoadedCount();
    }
...
{
    auto friend_list = UserMgr::Get_M_ptr()->GetConListPerPage();
    if (friend_list.empty() == false) {
        for(auto & friend_ele : friend_list){
        ...
        //更新已加载条目
        UserMgr::Get_M_ptr()->UpdateContactLoadedCount();
    }
}
